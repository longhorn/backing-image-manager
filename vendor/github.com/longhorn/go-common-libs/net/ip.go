...
func getIPForPodByNetworkPreference(podIP string,
	...
	if ip := net.ParseIP(podIP); ip != nil && ip.IsGlobalUnicast() {
		return ip.String(), nil
	}
...
// GetIPForPodByNetworkPreference returns the first usable global-unicast
// address from the storage network interface. If that interface is absent,
// it returns the validated and canonicalized POD_IP value.
func GetIPForPodByNetworkPreference() (ip string, err error) {
