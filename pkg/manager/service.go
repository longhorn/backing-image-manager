...
func (m *Manager) PrepareDownload(ctx context.Context, req *rpc.PrepareDownloadRequest) (resp *rpc.PrepareDownloadResponse, err error) {
	...
	// Use the primary pod IP for manager HTTP reverse-proxy reachability;
	// internal transfers continue using the storage-aware resolver above.
	address, err := util.GetBackingImageDownloadAddress(m.syncAddress)
	if err != nil {
